...
from data.io import open_scoremap, open_image
from jobs.logger import StandardOutputLogger, Logger
from utils import merge_dicts, clone
...
class GridSearchCV(BaseEstimator):
    def __init__(self, default_estimator, param_grid, cv, me, nontrainable_param_grid=None, scoring_rank='f1_score',
                 refit=False, iid=True, n_jobs=1, pre_dispatch='2*n_jobs',
                 logger=StandardOutputLogger(Logger.INFO)):
        self.default_estimator = default_estimator
        self.param_grid = param_grid
        self.nontrainable_param_grid = nontrainable_param_grid
        self.cv = cv
        self.me = me
        ...
        _check_param_grid(nontrainable_param_grid)
    ...
    def fit(self, X, y=None, groups=None):
        ...
        candidate_nontrainable_params = ParameterGrid(self.nontrainable_param_grid)
        ...
        results = dict()
        n_tot_candidates = n_candidates * n_nontrainable_candidates
        tot_candidate_params = list(itertools.product(list(candidate_params),list(candidate_nontrainable_params)))
        ...
        def _store(key_name, array, weights=None, splits=False, rank=False, error=False):
            ...
            if rank:
                ...
                results["rank_%s" % key_name] = np.asarray(
                        rankdata(arr, method='min'), dtype=np.int32)
...
def _fit_and_score(estimator, X, y, me, train, test, logger,
                   parameters, candidate_nontrainable_params,
    ...
    for nontrainable_parameters in candidate_nontrainable_params:
        ...
        for x, y in itertools.izip(X_test, y_test):
            ...
            pp = estimator.postprocessing([p], **nontrainable_parameters)
