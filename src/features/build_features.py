...
from jobs.logger import Logger, StandardOutputLogger, SilentLogger

__author__ = "Rubens Ulysse <urubens@student.ulg.ac.be>"
__version__ = '0.1'
...
class ColorSpaceBlue(ColorspaceGrayscale):
    def __init__(self, features):
        ColorspaceGrayscale.__init__(self, features)

    def build(self, image):
        if len(image.shape) != 3:
            image = cv2.cvtColor(image, cv2.COLOR_GRAY2RGB)

        self.image = image[:, :, 0]  # BGR
        return self
...
class FeaturesExtractor:
    def __init__(self, colorspaces, dtype=np.float16, logger=StandardOutputLogger(Logger.INFO)):
        ...
        for colorspace, features in colorspaces_dict.items():
            ...
            elif colorspace in ('blue', 'b', 'B'):
                self.colorspaces.append(ColorSpaceBlue(features))
