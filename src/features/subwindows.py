...
from features.build_features import FeaturesExtractor
from preprocessing import (
    scoremap_transform_edt,
    scoremap_transform_proximity,
    scoremap_transform_density
)
from data.io import open_image_with_mask, open_scoremap
...
def mk_subwindows(X, y, labels, sw_input_size=(5,5), sw_output_size=(1,1), sw_extr_mode='random',
                  sw_colorspace=['RGB__rgb'], mean_radius=2, pre_transformer=None, dtype=np.float16, flatten=True,
                  dataset_augmentation=False, return_labels=False, random_state=42,
    ...
    **kwargs
        Optional keyword arguments that have to be passed depending on the scoremap 
        transformer.
        If scoremap_transformer='edt' or 'proximity', **kwargs must contain:
        - alpha: int, controlling the exponential shape of the euclidean dist. transform
    ...
    for image_filename, groundtruth_filename, label in zip(X, y, labels):
        image, mask = open_image_with_mask(image_filename,
                                           padding=half_size(sw_input_size))
        scoremap = open_scoremap(groundtruth_filename,
                                 padding=half_size(sw_input_size))
        ...
        elif pre_transformer == 'proximity':
            if pre_alpha is None:
                raise ValueError('alpha parameter required/invalid')
            scoremap = scoremap_transform_proximity(scoremap, mean_radius, pre_alpha)
        elif pre_transformer == 'density':
            scoremap = scoremap_transform_density(scoremap, mean_radius)

        multispectral_image = FeaturesExtractor(sw_colorspace,
                                                dtype=dtype).build(image).feature_image

        __X, __y = extract_subwindows_image(multispectral_image, scoremap, mask,
                                            sw_input_size,
                                            sw_output_size, sw_extr_mode, mean_radius, flatten,
    ...
    # TODO: bug sometimes with GANGLIONS dataset ?!
...
def extract_subwindows_image(multispectral_image, scoremap, mask,
                             input_window_size, output_window_size, mode, mean_radius, flatten=True,
                             dataset_augmentation=False, random_state=42, sw_extr_stride=None, sw_extr_ratio=None,
                             sw_extr_score_thres=None, sw_extr_npi=None):
    """
    ...
    Parameters
    ----------
    multispectral_image: array-like of shape (width, height, n_features)
    ...
    **kwargs
    ...
    if mode == 'random':
        ...
        if sw_extr_stride is None:
            raise ValueError('stride parameter required/invalid')
        window_centers = _extract_sliding(mask, sw_extr_stride)
    ...
    for window_center in window_centers:
        top, right, bottom, left = subwindow_box(input_window_size,
                                                 ...
                                                 window_center)
        input_window = multispectral_image[slice(top, bottom), slice(left, right), :]
...
def _extract_scoremap_constrained(mask, scoremap, bg_ratio, score_threshold):
    ...
    n_additional_sw = min(int(bg_ratio * foreground_idxs.shape[0]),
                          background_idxs.shape[0])

    if n_additional_sw > 0:
        background_idxs = background_idxs[np.random.choice(background_idxs.shape[0],
                                                           n_additional_sw,
                                                           replace=False)]
    else:
        background_idxs = []

    return [tuple(idx) for idx in foreground_idxs] \
           + [tuple(idx) for idx in background_idxs]


def _extract_sliding(mask, stride):
    return []
...
def all_subwindows_generator(image, mask, sw_input_size, sw_colorspace,
    ...
    multispectral_image = FeaturesExtractor(sw_colorspace, dtype=dtype).build(image).feature_image
    window_centers = np.argwhere(np.asarray(mask) > 0)

    batch_subwindows = []
    batch_coords = []
    for i, window_center in enumerate(window_centers):
        top, right, bottom, left = subwindow_box(sw_input_size,
                                                 input_window_size_half,
                                                 window_center)
        batch_subwindows.append(
                multispectral_image[slice(top, bottom), slice(left, right), :].ravel())
...
if __name__ == '__main__':
    ...
    # rois = rois_from_dataset('ANAPATH',
    #                          working_path='/Users/ulysse/Documents/Programming/TFE/tmp/',
    #                          force_download=False)
    # for r in rois:
    #     X.append(r.image_filename)
    #     y.append(r.groundtruth_filename)
    #     labels.append(r.roi_id)
    #
    # _X, _y, _l = mk_subwindows(X, y, labels, (50, 50), (4, 4), 'scoremap_constrained',
    #                            ['RGB__RGB', 'L__grayscale'],
    #                            dataset_augmentation=True, bg_ratio=0.5,
    #                            score_threshold=0.5,
    #                            return_labels=True,
    #                            mean_radius=39,
    #                            dtype=np.uint8)
    ...
    # print _X.shape
    # print _y.shape
    # print "Number of subwindows: {}".format(_X.shape[0])
