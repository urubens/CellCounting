...
def sgd_compile(model, lr=1e-2, momentum=0.9, decay=0., nesterov=True, metrics=None):
    """
    ...
    nesterov : bool, optional
        Apply Nesterov momentum or not (default: True)
...
def FCRN_test(input_shape, regularizer=None, initializer='orthogonal', bn=False):
    model = Sequential()

    model.add(Conv2D(1, (3, 3), padding='same', use_bias=False, kernel_initializer=initializer,
                     kernel_regularizer=regularizer, input_shape=input_shape, name="1-Conv"))
    if bn:
        model.add(BatchNormalization(name="1-BatchNorm"))
    model.add(Activation(activation='relu', name="1-ReLU"))
...
class ParameterEstimation(Layer):
    ...
    #     mul = K.constant([[[self._n_rows, self._n_cols, 1] * self._M]])
    #     return tf.multiply(inputs, mul)
...
class SpatiallyConstrained(Layer):
    ...
    def call(self, inputs):
        return K.ones(shape=(1, 1), name="test")
